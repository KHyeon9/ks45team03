...
public class AdminCommisionRateService {
	...
	public Map<String, Object> adminGetCommisionRateList (int currentPage){
				...
				// 마지막페이지 
				// 테이블의 전체 행의 갯수
				double rowCnt = adminCommisionRateMapper.getCommisionRateListCnt();
				// 마지막페이지
		...
		List<CommisionRate> adminGetCommisionRateList = adminCommisionRateMapper.adminGetCommisionRateList(paramMap);
		

		// controller에 전달하기 위한 파라미터 셋팅
		paramMap.clear();
		paramMap.put("adminGetCommisionRateList", adminGetCommisionRateList);
